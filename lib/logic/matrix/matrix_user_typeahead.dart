...
class _MatrixUserTypeAhead extends StatelessWidget {
  const _MatrixUserTypeAhead(
      {Key? key, required this.onSelect, this.excludeUsers = const []})
      : super(key: key);
  ...
  Future<List<Profile>> searchUser(String search) async {
    ...
    var result =
        (await AngerApp.matrix.client.searchUserDirectory(search)).results;
    return result
        .where((element) => !excludedUserIds.contains(element.userId))
        .toList();
  }
    ...
    return TypeAheadField<Profile>(
        ...
        itemBuilder: (context, itemData) {
          return ListTile(
              ...
              subtitle:
                  itemData.displayName == null ? null : Text(itemData.userId));
        },
        onSuggestionSelected: (suggestion) {
          logger.d("Selected user: " + suggestion.userId);
          onSelect(suggestion);
        });
  }
}
